...
public class CSVParser {
    ...
    public CSVParser(IEnumerable<DATAFile> files, AlteDBXMLLibrary alteDB) {
    ...
    public void ParseReihenOrteExemplareBaende() {
        ...
        foreach (var n in toparse) {
            ...
            string[]? status = null;
            if (vorh != null) {
                if (vorh == "Original") status = new string[] { "Original vorhanden" };
                if (vorh == "Reprint") status = new string[] { "Reprint vorhanden" };
                if (vorh == "fremde Herkunft") status = new string[] { "Fremde Herkunft" };
                if (vorh == "Reprint u. fremde Herkunft") status = new string[] { 
                    "Reprint vorhanden",
                    "Fremde Herkunft"
                };
            }
        ...
        foreach (var n in werkehrsg) {
            ...
            if (werke.ContainsKey(n.Key)) {
                if (n.Value.Count >= 1) {
                    werke[n.Key].HRSGID1 = this.Akteure.Where(x => x.NAME == n.Value[0]).First().ID;
                }
                if (n.Value.Count >= 2) {
                    werke[n.Key].HRSGID2 = this.Akteure.Where(x => x.NAME == n.Value[1]).First().ID;
                }
                if (n.Value.Count >= 3) {
                    werke[n.Key].HRSGID3 = this.Akteure.Where(x => x.NAME == n.Value[2]).First().ID;
                }
                if (n.Value.Count >= 4) {
                    werke[n.Key].HRSGID4 = this.Akteure.Where(x => x.NAME == n.Value[3]).First().ID;
                }
                if (n.Value.Count >= 5) {
                    werke[n.Key].HRSGID5 = this.Akteure.Where(x => x.NAME == n.Value[4]).First().ID;
                }
            }
        }
    ...
    private void SaveFile(string fileroot) {
        var opt = new CsvConfiguration(CultureInfo.CurrentCulture) {
            MemberTypes = MemberTypes.Fields
        };
        using (var sw = new StreamWriter(fileroot + "akteure.csv"))
        using (var csvw = new CsvWriter(sw, opt)) {
            csvw.WriteRecords(this.Akteure);
        }
        using (var sw = new StreamWriter(fileroot + "baende.csv"))
        using (var csvw = new CsvWriter(sw, opt)) {
            // csvw.Context.RegisterClassMap<BaendeMap>();
            csvw.WriteRecords(this.Baende);
        }
    }
